...
fn add_item_duplicate_test() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PeaqStorage::add_item(Origin::signed(origin), ITEM_TYPE.to_vec(), ITEM.to_vec()),
...
fn get_non_existing_item_test() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PeaqStorage::get_item(Origin::signed(origin), b"new_item_type".to_vec()),
            Error::<Test>::ItemNotFound
        );
    });
}
...
fn get_other_owner_item_test() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PeaqStorage::get_item(Origin::signed(fake_origin), ITEM_TYPE.to_vec()),
            Error::<Test>::ItemNotFound
        );
    });
}
